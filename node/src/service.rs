...
pub fn new_partial(
    ...
    sc_service::PartialComponents<
        ...
        (
            impl Fn(sample_rpc::DenyUnsafe, sc_rpc::SubscriptionTaskExecutor) -> sample_rpc::IoHandler,
            (
...
> {
    ...
    let (rpc_extensions_builder, rpc_setup) = {
        ...
        let rpc_extensions_builder = move |deny_unsafe, subscription_executor| {
            let deps = sample_rpc::FullDeps {
                client: client.clone(),
                pool: pool.clone(),
                select_chain: select_chain.clone(),
                chain_spec: chain_spec.cloned_box(),
                deny_unsafe,
                babe: sample_rpc::BabeDeps {
                    babe_config: babe_config.clone(),
                    shared_epoch_changes: shared_epoch_changes.clone(),
                    keystore: keystore.clone(),
                },
                grandpa: sample_rpc::GrandpaDeps {
...
> {
    ...
    let light_deps = sample_rpc::LightDeps {
    ...
    let rpc_extensions = sample_rpc::create_light(light_deps);
